...
/*
    ...
    A grid is defined by rows_split and cols_split. More precisely, a matrix is split
    into blocks such that a block with coordinates (i, j) is defined by:
...
interval grid2D::row_interval(int index) const {
    if ((size_t) index >= rows_split.size() - 1) {
        throw std::runtime_error("ERROR: in class grid2D, row index out of range.");
    }
    return {rows_split[index], rows_split[index + 1]};
}

// returns index-th column interval, i.e. [cols_split[index], cols_split[index + 1])
interval grid2D::col_interval(int index) const {
    if ((size_t) index >= cols_split.size() - 1) {
        throw std::runtime_error("ERROR: in class grid2D, col index out of range.");
    }
...
*/
assigned_grid2D::assigned_grid2D(grid2D&& g, std::vector<std::vector<int>>&& proc, int n_ranks):
...
// returns a grid
const grid2D& assigned_grid2D::grid() const {
    return g;
}
...
int assigned_grid2D::num_ranks() const {
    return n_ranks;
}
...
int assigned_grid2D::block_size(int row_index, int col_index) {
    return rows_interval(row_index).length() * cols_interval(col_index).length();
}
}
